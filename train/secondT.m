function [p,A2,O,A,E,w_max2,w_min2,K,F2,Q,TTT2] = secondT( ~ )
%UNTITLED12 第二辆列车初始化数据
%   分别给出了w变量的上下限值， 等式约束中A的取值，常量向量E的取值，不等式约束A2的构造，目标函数半正定矩阵Q的构造以及
%   x'Qx+px中p向量的构造
u_min = -110000.*ones(180,1); %牵引力下限值,构造相应的180维行向量 值为-110000
%先构造x1的约束
% a1 = zeros(96,1); %1-2站台间[x1,x2,v]变量的下限 x1范围
% t = [130000 0 0]';
...
% %构造x2 的约束
% u_min2 = zeros(180,1);
ax1 = zeros(111,1); %1-2站台间[x1,x2,v]变量的下限 x1范围
t = [0 135000 0]';
...
% t = [130000 0 0]';
% a1 = repmat(t,32,1); %1-2站台间[x,v]变量的下限
% t = [260000 0 0]';
% a2 = repmat(t,32,1);%3-4站台间[x,v]变量的下限
% t = [390000 0 0]';
% a3 = repmat(t,32,1);%4-5站台间[x,v]变量的下限
% t = [520000 0 0]';
% a4 = repmat(t,32,1);%5-6站台间[x,v]变量的下限
% t = [650000 0 0]';
% a5 = repmat(t,32,1);
% t = [650000 0 0]';
% a6 = repmat(t,20,1);
% w_x1 = [u_max;a1;a2;a3;a4;a5;a6]; %720维的列向量
% % 构造x2 的约束
% u_min2 = zeros(180,1);
ax1 = [675000 0 0]';
ax1 = repmat(ax1,5,1);%1-2站台间[x1,x2,v]变量的下限 x1范围
...
Z2 = blkdiag(Z1{:});
blockA2z1 = zeros(5,15);
blockA2z2 = zeros(15,45);
