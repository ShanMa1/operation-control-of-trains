function [p,A2,O,A,E,w_max4,w_min4,K,F4,Q,TTT4 ] = fourthT(~)
%UNTITLED19 此处显示有关此函数的摘要
%   此处显示详细说明
u_min = -110000.*ones(180,1); %牵引力下限值,构造相应的180维行向量 值为-110000
%先构造x1的约束
% a1 = zeros(126,1); %1-2站台间[x1,x2,v]变量的下限 x1范围
% t = [130000 0 0]';
...
% %构造x2 的约束
% u_min2 = zeros(180,1);
ax1 = zeros(141,1); %1-2站台间[x1,x2,v]变量的下限 x1范围
t = [0 135000 0]';
...
% ax1 = zeros(30,1); %1-2站台间[x1,x2,v]变量的下限 x1范围
% t = [130000 0 0]';
% ax2 = repmat(t,32,1);%2-3站台间[x,v]变量的下限
% t = [260000 0 0]';
% ax3 = repmat(t,32,1);%3-4站台间[x,v]变量的下限
...
Z2 = blkdiag(Z1{:});
blockA2z1 = zeros(15,45);
blockA2z2 = zeros(5,15);
...
f2 = blkdiag(f2{:});
F4 = [zeros(360,360) f1 f2 zeros(360,180)];
TTT4 = c;
end
